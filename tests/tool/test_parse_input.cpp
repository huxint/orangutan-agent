...
TEST_CASE("parse_input_object rejects malformed JSON with tool-prefixed detail", "[unit][tool][parse_input]") {
  const auto result = detail::parse_input_object("{not-json", "demo.tool");

  REQUIRE_FALSE(result.has_value());
  REQUIRE(result.error().kind() == core::ErrorKind::invalid_argument);
  REQUIRE(result.error().message() == "demo.tool: input is not valid JSON");
...
TEST_CASE("parse_input_object rejects non-object top-level JSON", "[unit][tool][parse_input]") {
  const auto array_result = detail::parse_input_object("[1,2,3]", "demo.tool");
  REQUIRE_FALSE(array_result.has_value());
  REQUIRE(array_result.error().kind() == core::ErrorKind::invalid_argument);
  REQUIRE(array_result.error().message() == "demo.tool: input must be a JSON object");

  const auto string_result = detail::parse_input_object("\"a string\"", "demo.tool");
  REQUIRE_FALSE(string_result.has_value());
  REQUIRE(string_result.error().message() == "demo.tool: input must be a JSON object");
}

TEST_CASE("parse_input_object returns the parsed object on success", "[unit][tool][parse_input]") {
  const auto result = detail::parse_input_object(R"({"path":"a","count":3})", "demo.tool");
...
TEST_CASE("require_string_field returns the field value on success", "[unit][tool][parse_input]") {
  const auto parsed = detail::parse_input_object(R"({"path":"x.txt"})", "demo.tool");
  REQUIRE(parsed.has_value());

  const auto path = detail::require_string_field(*parsed, "demo.tool", "path");
  REQUIRE(path.has_value());
  REQUIRE(*path == "x.txt");
}

TEST_CASE("require_string_field rejects missing fields", "[unit][tool][parse_input]") {
  const auto parsed = detail::parse_input_object(R"({"other":"value"})", "demo.tool");
  REQUIRE(parsed.has_value());

  const auto path = detail::require_string_field(*parsed, "demo.tool", "path");
  REQUIRE_FALSE(path.has_value());
  REQUIRE(path.error().kind() == core::ErrorKind::invalid_argument);
  REQUIRE(path.error().message() == "demo.tool: input must include a string `path` field");
}

TEST_CASE("require_string_field rejects non-string fields", "[unit][tool][parse_input]") {
  const auto parsed = detail::parse_input_object(R"({"path":42})", "demo.tool");
  REQUIRE(parsed.has_value());
...
TEST_CASE("require_string_field is tool-name agnostic", "[unit][tool][parse_input]") {
  const auto parsed = detail::parse_input_object(R"({"content":""})", "file.write");
  REQUIRE(parsed.has_value());

  const auto missing = detail::require_string_field(*parsed, "file.write", "path");
  REQUIRE_FALSE(missing.has_value());
  REQUIRE(missing.error().message() == "file.write: input must include a string `path` field");

  const auto empty_ok = detail::require_string_field(*parsed, "file.write", "content");
