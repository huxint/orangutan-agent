...
| Scheduler path locks | Give idle-entry reclamation a production owner; `reap_idle_locks` currently has only test callers. Narrow unused controls while retaining shared path exclusion and context-specific joins. | Bounded entry retention under changing paths, read/write exclusion and cancellation tests. |
