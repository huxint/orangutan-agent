...
| Permissions              | C     | Foundation `oran-permission` library is implemented: `Verdict { allow, deny, ask }`, `Mode { strict, default_, permissive, sandboxed }`, `Rule` (with an optional `core::Capability` scope), `Decision`, `RuleSet` with deny → allow → ask precedence, a `*`-glob tool-name matcher, a capability-aware `evaluate(tool, required_capabilities, mode)` overload (legacy `evaluate(tool, mode)` retained as a wrapper that passes `{}`), a `Defaults::for_mode(Mode)` baseline factory (layer-1 of the three-layer rule merge), and the three-layer `materialize(Mode, global, per_agent) -> RuleSet` that concatenates defaults + global `config::PermissionsConfig` + per-agent overlay (the runtime merge the design doc describes is now end-to-end live). Tests grew to 30 cases / 114 assertions covering literal + wildcard match, deny-wins precedence, capability-bound match / miss / fallback, indexed-reason output (annotated with `capability=<name>` when the rule was scoped), the per-mode baseline shapes, and the three-layer concatenation semantics (empty merge == defaults, layer order preserved, verdict mapping one-to-one, deny in any layer outranks allow in any other layer, capability scope preserved through materialize). Bench grew to a three-layer `materialize` A/B alongside the precedence walk, capability match/miss, and `Defaults::for_mode` factory-vs-inline. re2 input regex, HMAC approvals, audit logging, and bootstrap wiring are still downstream. | Land re2 input-pattern matching for `Rule::input_pattern` per `0008-permissions.md` criterion 4. |
