...
| Test framework           | C     | Catch2 v3 buckets green for `oran-core` (54 cases / 366 assertions), `oran-async` (8 cases / 38 assertions), `oran-io` (8 cases / 33 assertions), `oran-storage` (52 cases / 606 assertions), `oran-config` (5 cases / 49 assertions), `oran-permission` (15 cases / 59 assertions), `oran-cli` (5 cases / 30 assertions), and `oran-bootstrap` (8 cases / 34 assertions). | Add first tool / hook tests when those libraries land. |
| Bench harness            | C     | nanobench buckets green for `oran-core`, `oran-async`, `oran-io`, `oran-storage`, `oran-config`, `oran-permission`, `oran-cli`, and `oran-bootstrap`; core covers builder-vs-prefilled `Error` construction, time format-explicit-vs-chrono-format plus a parse cost scenario, Content visit-vs-get_if traversal plus a 32-block message walk, `ToolDef` aggregate-init-vs-`with_no_input` helper, the strict RFC-3629 `is_valid_utf8` walk vs. an ASCII-only short-circuit, and the `Capability` linear-scan parse vs. `std::unordered_map` lookup batch; storage covers inserts, compiled-span migrations, SQL-file migration load+run, direct-vs-pool acquire, fresh-vs-cached prepare, pool fresh-vs-cached prepare, and raw-pool-vs-session-repository append/load; permission covers `RuleSet::evaluate` precedence walk vs. a single-pass `std::ranges::find_if` plus a capability-aware match-vs-miss A/B; config covers parse-vs-file-load, CLI covers prompt-vs-REPL dispatch, and bootstrap covers default-vs-explicit config startup. No baseline JSON or regression gate yet. | Wire `scripts/bench-compare.sh` to a real baseline file. |
...
| Permissions              | C     | Foundation `oran-permission` library is implemented: `Verdict { allow, deny, ask }`, `Mode { strict, default_, permissive, sandboxed }`, `Rule` (now with an optional `core::Capability` scope), `Decision`, `RuleSet` with deny → allow → ask precedence, a `*`-glob tool-name matcher, and a capability-aware `evaluate(tool, required_capabilities, mode)` overload (legacy `evaluate(tool, mode)` retained as a wrapper that passes `{}`). Tests grew to 15 cases / 59 assertions covering literal + wildcard match, deny-wins precedence, capability-bound match / miss / fallback, and the indexed-reason output (now annotated with `capability=<name>` when the rule was scoped). Bench grew to a capability match-vs-miss A/B alongside the precedence walk. re2 input regex, HMAC approvals, audit logging, and config wiring are still downstream. | Add HMAC-signed approval prompts on top of the capability foundation. |
