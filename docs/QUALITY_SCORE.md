...
| Test framework           | C     | Catch2 v3 buckets green for `oran-core` (8 cases / 47 assertions), `oran-async` (8 cases / 38 assertions), `oran-io` (8 cases / 33 assertions), `oran-storage` (16 cases / 150 assertions), `oran-config` (5 cases / 49 assertions), and `oran-bootstrap` (5 cases / 29 assertions). | Add first permission/tool tests when those libraries land. |
| Bench harness            | C     | nanobench buckets green for `oran-core`, `oran-async`, `oran-io`, `oran-storage`, `oran-config`, and `oran-bootstrap`; storage covers inserts and migrations, config covers parse-vs-file-load, bootstrap covers default-vs-explicit config startup. No baseline JSON or regression gate yet. | Wire `scripts/bench-compare.sh` to a real baseline file. |
...
| Storage / DBs            | C     | Expected-only SQLite `Connection` / `Statement` core and migration runner are implemented with WAL/foreign-key setup, schema versioning, tests, and benches. Pooling, SQL-file loading, statement cache, and domain repositories are still downstream. | Add storage pool or start `oran-config` on top of migrations. |
...
| Bootstrap                | C     | Process entry now routes through `oran-bootstrap`, loads explicit or default config, falls back to built-in defaults when the default file is absent, and has tests/bench coverage. Runtime assembly, signal handling, REPL, and provider startup are still downstream. | Add `oran-cli` handoff and runtime assembly around loaded config. |
