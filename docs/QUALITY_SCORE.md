...
| Test framework           | C     | Catch2 v3 buckets green for `oran-core` (8 cases / 47 assertions), `oran-async` (8 cases / 38 assertions), `oran-io` (8 cases / 33 assertions), `oran-storage` (25 cases / 229 assertions), `oran-config` (5 cases / 49 assertions), `oran-cli` (5 cases / 30 assertions), and `oran-bootstrap` (8 cases / 34 assertions). | Add first permission/tool tests when those libraries land. |
| Bench harness            | C     | nanobench buckets green for `oran-core`, `oran-async`, `oran-io`, `oran-storage`, `oran-config`, `oran-cli`, and `oran-bootstrap`; storage covers inserts, migrations, and direct-vs-pool acquire; config covers parse-vs-file-load, CLI covers prompt-vs-REPL dispatch, and bootstrap covers default-vs-explicit config startup. No baseline JSON or regression gate yet. | Wire `scripts/bench-compare.sh` to a real baseline file. |
...
| Storage / DBs            | C     | Expected-only SQLite `Connection` / `Statement` core, migration runner, and async writer/reader `Pool` are implemented with WAL/foreign-key setup, schema versioning, RAII leases, FIFO waiter resumption, cancellation, tests, and benches. Statement cache, SQL-file loading, and domain repositories are still downstream. | Add prepared-statement cache or first domain repository on top of the pool. |
