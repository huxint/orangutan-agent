...
| Permissions              | C     | Foundation `oran-permission` library now covers four axes: (1) `Verdict { allow, deny, ask }`, `Mode { strict, default_, permissive, sandboxed }`, `Rule`, `Decision`, and `RuleSet` with the deny → allow → ask precedence walk and a `*`-glob tool-name matcher; (2) capability-aware gating via the optional `Rule::capability` (of `core::Capability`) and the capability-aware `RuleSet::evaluate(tool, required_capabilities, mode)` overload (legacy `evaluate(tool, mode)` retained as a wrapper that passes `{}`); (3) the `Defaults::for_mode(Mode)` baseline factory (layer-1 of the three-layer rule merge); (4) the three-layer `materialize(Mode, global, per_agent) -> RuleSet` that concatenates defaults + global `config::PermissionsConfig` + per-agent overlay (the runtime merge the design doc describes is end-to-end live); (5) runtime input regex via `permission::InputPattern` (re2 partial match) — rules carry an optional `Rule::input_pattern`, and the four-argument `RuleSet::evaluate(tool, input, required_capabilities, mode)` overload pays re2's match cost only when the rule is scoped to it. Tests grew to 39 cases / 149 assertions covering literal + wildcard match, deny-wins precedence, capability-bound match / miss / fallback, indexed-reason output (annotated with `capability=<name>` and `input=~"<pattern>"` when the rule was scoped), the per-mode baseline shapes, the three-layer concatenation semantics (empty merge == defaults, layer order preserved, verdict mapping one-to-one, deny in any layer outranks allow in any other layer, capability scope preserved through materialize), the re2 compile success / failure paths (re2's error message attached as the `regex_error` context entry on `Error::invalid_argument`), partial-match semantics, and the capability + input_pattern composition on a single rule. Bench grew to an `input_pattern` match / miss / no-pattern A/B alongside the precedence walk, capability match/miss, `Defaults::for_mode` factory-vs-inline, and the three-layer materialize A/B. HMAC approvals, audit logging, and bootstrap wiring are still downstream. | Wire `Rule::input_pattern` through `oran-config` and `materialize` next, then land HMAC-signed approval prompts per `0008-permissions.md` criterion 5. |
