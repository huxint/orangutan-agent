...
| Permissions              | C     | Foundation `oran-permission` library is implemented: `Verdict { allow, deny, ask }`, `Mode { strict, default_, permissive, sandboxed }`, `Rule` (with an optional `core::Capability` scope), `Decision`, `RuleSet` with deny → allow → ask precedence, a `*`-glob tool-name matcher, a capability-aware `evaluate(tool, required_capabilities, mode)` overload (legacy `evaluate(tool, mode)` retained as a wrapper that passes `{}`), and a `Defaults::for_mode(Mode)` baseline factory (layer-1 of the three-layer rule merge: empty for `strict`, capability-scoped allow/ask/deny baseline for `default_`, deny-the-dangerous for `permissive`, read-side-only allow for `sandboxed`). Tests grew to 22 cases / 91 assertions covering literal + wildcard match, deny-wins precedence, capability-bound match / miss / fallback, indexed-reason output (annotated with `capability=<name>` when the rule was scoped), and the per-mode baseline shapes. Bench grew to a `Defaults` factory-vs-inline A/B alongside the precedence walk and capability match/miss. re2 input regex, HMAC approvals, audit logging, and config wiring are still downstream. | Wire `Defaults::for_mode` plus config-driven rules into `oran-config::permissions` (layer-2 merge). |
