...
| Test framework           | C     | Catch2 v3 buckets green for `oran-core` (8 cases / 47 assertions), `oran-async` (8 cases / 38 assertions), `oran-io` (8 cases / 33 assertions), `oran-storage` (16 cases / 150 assertions), and `oran-config` (5 cases / 46 assertions). | Add first permission/tool tests when those libraries land. |
| Bench harness            | C     | nanobench buckets green for `oran-core`, `oran-async`, `oran-io`, `oran-storage`, and `oran-config`; storage covers inserts and migrations, config covers parse-vs-file-load. No baseline JSON or regression gate yet. | Wire `scripts/bench-compare.sh` to a real baseline file. |
