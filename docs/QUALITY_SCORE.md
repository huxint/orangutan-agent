...
| Test framework           | C     | Catch2 v3 buckets green for `oran-core` (8 cases / 47 assertions), `oran-async` (8 cases / 38 assertions), `oran-io` (8 cases / 33 assertions), and `oran-storage` (16 cases / 150 assertions). | First config tests. |
| Bench harness            | C     | nanobench buckets green for `oran-core`, `oran-async`, `oran-io`, and `oran-storage`; storage covers inserts and migrations. No baseline JSON or regression gate yet. | Wire `scripts/bench-compare.sh` to a real baseline file. |
