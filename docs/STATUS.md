...
- **Slice:** 191 (`xmake run orangutan -- --help` reports slice 191)
- **Last completed history:**
  [`histories/2026-06/20260607-1035-automation-retention-decay-hooks.md`](histories/2026-06/20260607-1035-automation-retention-decay-hooks.md)
- **Active exec-plan:**
...
- **Latest completed slice:** slice 191 makes the explicit retention tick owner
  the first periodic `memory_decay` producer. `MemoryRetentionService` now
  accepts optional hook settings (`hook::Bus*`, source label, agent key, and
  identity); after a successful due tick records the run and advances
  `last_fired_at`, it publishes advisory metadata reusing the planned
  `DecayRequest` values and reports sink/failure counts in the tick result.
  Not-due ticks and backend failures publish nothing, and advisory sink failures
  do not roll back the durable run. It still does not start a scheduler,
  acquire leases, publish job lifecycle hooks, or wire bootstrap to open
  `automation.db`. Focused result: `test-automation`
  **18 cases / 207 assertions**.
- **Next intended slice:** continue the active automation-retention cadence
  ...
  boundary is explicit service-loop ownership around the retention tick:
  caller-owned `automation.db` opening, lease/cancellation policy, and job
  lifecycle hooks, while still avoiding a hidden bootstrap background loop
...
Slice 183 adds the operator-facing long-term
  ...
  execution, and slice 191 publishes periodic `memory_decay` metadata from that
  explicit tick owner when a caller supplies a hook bus, while bootstrap
  ...
  and invokes the supplied long-term backend only when due, and slice 191 lets
  that tick publish advisory `memory_decay` metadata after successful due
  retention, while bootstrap still does not open or run `automation.db`.
...
Slice 182 adds the library-level long-term decay
  ...
  `MemoryRetentionService`, and periodic advisory `memory_decay` publishing
  landed in slice 191 when callers provide a hook bus. Startup hook publishing
  landed in slice 186.
  Focused result:
