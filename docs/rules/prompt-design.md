...
Slices 135-143 land the first deterministic section-4 owner, loader snapshot,
...
markers plus an explicit `skill::ActivationPolicy` resolver in `oran-skill`,
`oran-tool`, and bootstrap. That owner keeps skill bodies out of section 1 and
renders a compact metadata-only catalog before loop entry; `skill.invoke`
returns body text as an ordinary conversation-tail tool result, so it does not
mutate cached sections (1)-(6) during an active turn. Watcher / signature
refresh, per-agent allowlist filtering, and successful activation metadata
replace section-4 bytes before the next prompt when skill files, selected agent
config, or loaded transcript state change.
