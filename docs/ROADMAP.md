...
| **Tool scheduler** | Shipped (116–120): bounded parallelism, per-path read/write locks, per-call timeout, cancellation grace, production loop wiring. Slice 255 closes AC10's background tick: `--serve` shares one strand-hosted `ToolScheduler` across automation jobs and races a `serve_scheduler_reaping` concern that periodically reaps the idle-lock table; `AgentPromptRunner` now optionally borrows an injected `{registry, scheduler}` pair. | v1.1: dispatch singleflight + persisted index caches. Also drive the CLI / channel agent loops on a per-agent strand so the single-strand lock-table contract holds there as it now does under `--serve`. | — | [spec 0012](product-specs/0012-tool-scheduler-and-state.md) |
...
2. **Runtime-service / daemon owner** — *resolved 2026-06-20*: the owner is
   ...
   **scheduler idle-lock reaping tick** — `--serve` shares one strand-hosted
   ...
   three legs are shipped; non-CLI agent-loop entry via channel receive/dispatch can
   later ride the same owner. The optional Slice D (a typed `serve` config block) is
   deferred until more than one concern wants tuning.
