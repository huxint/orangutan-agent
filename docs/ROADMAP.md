...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 228: config-authored ingress — typed `channels[]` block, `register_configured_channels` (caller-owned manager, unknown kinds skipped), `make_routed_channel_prompt_runner` (per-channel agent routing); end-to-end config→adapter→agent→reply proof on top of the slice-227 `MockChannel` + dispatch seam. | First QQ-port slice: API client over `oran-http` behind the shipped trait. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) |
