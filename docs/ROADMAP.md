...
| **Observability & trace** | Slice 240 extends spec-0018 v1.1 trace export into a bounded stdout operator surface: `--trace-export <turn-id>` still emits one known turn, while `--trace-export [--agent <name>] [--limit <n>]` lists newest turns, optionally scoped by agent, and emits one redacted `kind=trace_turn` JSON Lines object per turn with joined audit rows. | Add a file sink for the same bounded trace-export query before considering HTTP posting or a broader query language. | — | [spec 0018](product-specs/0018-first-loop-observability.md) · [`RELIABILITY.md`](RELIABILITY.md) |
