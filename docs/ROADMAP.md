...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 256: `orangutan --serve` is now the first long-lived owner for configured channel ingress. Buildable `config.channels[]` entries register into a strand-owned `ChannelManager`, start under the service body, pump adapter `next_message()` results into fan-in, dispatch through the routed channel agent bridge, reply through the owning adapter, and stop/drain adapters on shutdown. The QQ real-smoke gate from slice 238 remains open and `channel_qq` stays default-off. | QQ-port milestone 4b-ii once credentials exist; otherwise generic hardening: per-conversation serialization/deadlines or webhook-triggered ingress. | Real QQ bot credentials and a sendable operator conversation for the QQ smoke run; no hard blocker for generic channel hardening. | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
| **Agent loop (ReAct)** | v1 shipped: seven-section prompt build, provider re-entry tool loop, scheduler-batched parallel dispatch (slice 120), trace rows, provider lifecycle hooks (126), profile-priced cost fill (129). Slice 256 adds the first non-CLI long-lived entry: configured channel messages under `--serve` dispatch through the routed `ChannelPromptRunner` bridge into the same agent loop. | Harden non-CLI ordering: per-conversation channel serialization/deadlines and per-agent strand ownership where the scheduler contract needs finer granularity than the service strand. | — | [spec 0001](product-specs/0001-core-react-loop.md) · [spec 0017](product-specs/0017-fake-provider-first-agent-loop.md) · [`design-docs/agent-platform.md`](design-docs/agent-platform.md) |
...
| **Tools — registry & built-ins** | Seven filesystem/search built-ins + `ToolSearch` + memory/skill built-ins; structured output, usage counters, byte caps (spec 0014 closed for built-ins); shared `parse_input` helpers (115); slice 245 public names are PascalCase (`FileRead`, `MemoryRecall`, `SkillInvoke`, etc.) instead of dotted tool names. | Re-shape `FileDelete` + `DirectoryList` into one unified delete tool and a recursive whole-project list. | None hard; meanwhile do not add new per-kind splits. | [`design-docs/tool-runtime.md`](design-docs/tool-runtime.md) · [spec 0002](product-specs/0002-tool-registry.md) · [spec 0014](product-specs/0014-structured-tool-output.md) |
| **Tool scheduler** | Shipped (116–120): bounded parallelism, per-path read/write locks, per-call timeout, cancellation grace, production loop wiring. Slice 255 closes AC10's background tick: `--serve` shares one strand-hosted `ToolScheduler` across automation jobs and races a `serve_scheduler_reaping` concern that periodically reaps the idle-lock table; `AgentPromptRunner` now optionally borrows an injected `{registry, scheduler}` pair. Slice 256 runs the configured-channel `--serve` agent bridge on the same service strand, though it still lacks per-agent/per-conversation serialization. | v1.1: dispatch singleflight + persisted index caches. Also drive the CLI agent loop on a per-agent strand and split channel dispatch into per-conversation/per-agent strands where needed. | — | [spec 0012](product-specs/0012-tool-scheduler-and-state.md) |
...
| **Automation** | Library-level retention/cron/triggered with durable state, leases, queues, caller-owned finite loops (187–225, plus 183–186 retention prework). Slice 254 gives it a long-lived driver: `orangutan --serve` opens `<workspace>/.orangutan/automation.db`, applies config `automation.cron.jobs[]` seeds once, and drives `AutomationService::run(...)` in a cancel-aware poll loop over a prompt-backed handler (live `HttpProviderBackend`, else offline scripted `FakeProvider`). Slice 256 gives `--serve` a configured-channel ingress loop, but that loop dispatches directly to agents rather than enqueueing triggered automation work. | A triggered-work ingress (channel/webhook → `enqueue_triggered`) so the triggered half of the `--serve` loop has a producer; the library itself stays caller-owned. | — (runtime-service owner + automation loop shipped, slice 254; generic channel loop shipped, slice 256). | [`design-docs/automation-runtime.md`](design-docs/automation-runtime.md) · [spec 0006](product-specs/0006-automation.md) · [runtime-service-owner plan](exec-plans/completed/2026-06-18-runtime-service-owner.md) · [completed plan](exec-plans/completed/2026-06-07-automation-cron-category.md) |
...
2. **Runtime-service / daemon owner** — *resolved 2026-06-20*: the owner is
   ...
   **scheduler idle-lock reaping tick** — `--serve` shares one strand-hosted
   ...
   on shutdown. The optional typed `serve` config block is deferred until more than one
   concern wants tuning.
