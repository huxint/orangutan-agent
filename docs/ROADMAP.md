...
| **Observability & trace** | Slice 239 starts spec-0018 v1.1 trace export: alongside the human `--trace <turn-id>` inspector, `--trace-export <turn-id>` emits one JSON Lines object containing the trace row plus joined audit rows from `<workspace>/.orangutan/audit.db`. The command is read-only, stdout-only, single-turn scoped, and preserves the existing redacted trace/audit contract. | Extend trace export into a bounded multi-turn operator surface before adding file or HTTP sinks. | — | [spec 0018](product-specs/0018-first-loop-observability.md) · [`RELIABILITY.md`](RELIABILITY.md) |
