...
| --- | --- | --- | --- | --- |
| **Channels** | Slice 259: `orangutan --serve` owns configured channel ingress with per-channel+conversation dispatch queues. Buildable `config.channels[]` entries register into a strand-owned `ChannelManager`, start under the service body, pump adapter `next_message()` results into fan-in, enqueue matching automation triggers when configured, then dispatch each `(channel_id, conversation_id)` key through its own bounded worker so one slow conversation does not block unrelated conversations. The QQ real-smoke gate from slice 238 remains open and `channel_qq` stays default-off. | QQ-port milestone 4b-ii once credentials exist; otherwise generic hardening: per-message deadlines/idle worker eviction or webhook-triggered ingress. | Real QQ bot credentials and a sendable operator conversation for the QQ smoke run; no hard blocker for generic channel hardening. | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
...
2. **Runtime-service / daemon owner** — *resolved 2026-06-20*: the owner is
   ...
   **scheduler idle-lock reaping tick** — `--serve` shares one strand-hosted
   ...
   unrelated conversations can proceed concurrently. Webhook/non-chat external
