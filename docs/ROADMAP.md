...
| **Desktop app** | Slices A–B shipped (248–249): prebuilt `slint` package + `--desktop` option + gated shell + `.slint`→C++ codegen + skeleton window (A); `web`→`desktop` config migration — `DesktopConfig{enabled,theme,reduce_motion}` replacing `WebConfig` (B). | Slice C — always-built `oran-desktop` bridge + `ChatViewModel` + `DesktopEventSink`, injecting `provider::EventSink*` into `AgentPromptRunner`; `tests/desktop` ≥60%. | None blocking; Slice D (chat tracer end-to-end) follows C. | [spec 0007](product-specs/0007-web-ui.md) · [`DESKTOP.md`](DESKTOP.md) · [active plan](exec-plans/active/2026-06-14-oran-desktop-chat-tracer.md) |
