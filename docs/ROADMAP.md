...
| **Workspace & path policy** | Registry pre-resolution provides pinned directory authority across approval, and every filesystem built-in executes through handle capabilities: `FileRead`, `FileWrite`, `FileEdit`, `FileDelete`, single-level `DirectoryList`, and — via `io::walk_directory_tree` — recursive `DirectoryList` and `FileSearch`. FileDelete pins the approved target inode before approval and performs recursive no-follow deletion through dirfds. `.gitignore` / `.ignore` files load through pinned scope authorities; the scheduler derives lock keys via the syscall-free `Workspace::lock_key`. The read resolver deliberately keeps a `refers_to_path`-gated pathname pass as its symlink normaliser (`RESOLVE_BENEATH` cannot follow absolute-target inside symlinks). Shared ignore/display behavior and the read/list-only outside-workspace override are shipped. | Optional future slice: an anchored `oran-io` canonicaliser so the read resolver's pathname pass can retire without dropping absolute inside-symlink support. | — | [spec 0013](product-specs/0013-workspace-and-path-policy.md) |
...
| **Build & CI** | `make ci` gates docs/hygiene/docs-sync/status/deps/preamble; supply-chain action pinning; docs-sync now also enforces public-header inventory + resolvable header refs, config.example.json top-level shape, hook Event catalogue/blocking-set sync, and Capability enum/doc-reference sync; `check-compile-budget.sh` exists but is unwired. | Wire `check-compile-budget.sh` plus xmake build/test into CI; add the clang-tidy hosted gate and the analyzer TU manifest/check. | Reference hardware provisioning (Dependency Frontier #3). | [`BUILD_SYSTEM.md`](BUILD_SYSTEM.md) · [`CICD.md`](CICD.md) · [`rules/compile-budget.md`](rules/compile-budget.md) |
