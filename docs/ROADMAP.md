...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 232: QQ-port milestone 2b-ii — `qq::GatewayTransport`, the caller-owned network driver that keeps a persistent `http::WebSocket`, drives `qq::GatewaySession` lifecycle frames, sends Identify/Resume/heartbeat payloads with tokens from `TokenStore`, races receive waits against an `async::sleep_for` heartbeat timer, reconnects with the documented backoff/close-code policy, and returns one non-lifecycle `GatewayDispatch` per `next_dispatch()` resume; gated `test-channel-qq` 45 cases / 249 assertions, no new dependency. | QQ-port milestone 3: introduce the first `qq::QqChannel` trait-adapter slice over `GatewayTransport` and `ApiClient`, starting with gateway dispatch normalization into `channel::InboundMessage` and the `Channel::next_message()` boundary. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
