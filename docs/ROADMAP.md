...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 230: QQ-port milestone 2a — the pure `qq::GatewaySession` protocol/session state machine (`consume(frame_json)` → `GatewayReaction`; Identify-vs-Resume on Hello, seq/session-id continuity, `build_identify`/`build_resume`/`build_heartbeat` with per-call token injection, `classify_close_code` with the SDK-grounded 4009/4013-4014/4914-4915 corrections), offline-tested with no new dependency; on top of slice-229's token store + API client. | QQ-port milestone 2b: the `wss://` network transport driving `GatewaySession` behind `Channel::next_message()` — a cancel-aware WebSocket boundary on `oran-http` (no WebSocket primitive exists yet; legacy used raw `curl_ws_*` on a `std::thread`, which C2/C6 forbid), persistent read loop, heartbeat timer over `async::sleep_for`, reconnect backoff. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
