...
2. **Runtime-service / daemon owner** — *resolved 2026-06-20*: the owner is
   ...
   **scheduler idle-lock reaping tick** — `--serve` shares one strand-hosted
   ...
   descriptors and `--serve` seed application; the external producer that enqueues
   channel/webhook events into `AutomationService` remains the next automation slice.
