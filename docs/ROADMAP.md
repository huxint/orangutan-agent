...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 236: QQ-port milestone 4a — the `--channel_qq=y` registered QQ path now round-trips under mock gateway/API coverage: bootstrap registers the configured adapter, `ChannelManager` receives one QQ gateway C2C message, the routed prompt bridge runs the configured agent, trace/audit state is written, and the passive QQ reply is sent through the v2 user-message API. Default builds still skip/report QQ entries without linking adapter code. | QQ-port milestone 4b: manual/nightly real-credential smoke over the same registered QQ path before considering `channel_qq` default-on. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
