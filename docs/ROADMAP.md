...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 234: QQ-port milestone 3b — `qq::QqChannel::send(...)` now sends passive text replies over `ApiClient`, selecting C2C/group v2 endpoints from `conversation_id`, serializing `content` / `msg_type:0` / `msg_id` / `msg_seq`, parsing delivery receipts, and rejecting unsupported outbound shapes before HTTP; generic `make_reply_message(...)` preserves the first inbound reply reference for passive replies; gated `test-channel-qq` 55 cases / 344 assertions. | QQ-port milestone 3c: register configured QQ channels in bootstrap behind `--channel_qq=y` and `channels[].kind == "qq"` before round-trip acceptance. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
