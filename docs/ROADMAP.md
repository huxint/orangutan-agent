...
| **Tools — registry & built-ins** | Seven filesystem/search built-ins + `ToolSearch` + memory/skill built-ins; structured output, usage counters, byte caps (spec 0014 closed for built-ins); shared `parse_input` helpers (115); slice 245 public names are PascalCase (`FileRead`, `MemoryRecall`, `SkillInvoke`, etc.) instead of dotted tool names. Slice 264 extends `DirectoryList` with `recursive=true` for whole-project tree listings under the existing tool name, keeping root workspace confinement, structured `data_json.recursive`, bounded `max_entries`, and built-in skips for `.git`, `.xmake`, `.orangutan`, `build`, and `node_modules`. | Finish the reshape by unifying `FileDelete` into a single delete tool that can cover files and folders with explicit recursion intent; do not add new per-kind splits. | None hard. | [`design-docs/tool-runtime.md`](design-docs/tool-runtime.md) · [spec 0002](product-specs/0002-tool-registry.md) · [spec 0014](product-specs/0014-structured-tool-output.md) |
...
| **Workspace & path policy** | v1 shipped (55): pre-resolution, confinement, redacted audit metadata. Slice 264 supplies the second recursive filesystem consumer through `DirectoryList recursive=true`; the shared ignore predicate is now a structure task, not blocked on another recursive tool. | v1.1 shared ignore predicate + display helpers across `FileSearch` and recursive `DirectoryList`. | — | [spec 0013](product-specs/0013-workspace-and-path-policy.md) |
...
3. **Reference hardware for CI** — gates `check-compile-budget.sh` in
   ...
   2026-05-21).
4. **Recursive list / unified delete reshape** — *partially resolved
   2026-06-28*: slice 264 ships `DirectoryList recursive=true`, supplying the
   second recursive consumer that unblocks workspace v1.1's shared ignore
   predicate. The unified delete half remains open on the Tools row.
