...
| **Tool scheduler** | Shipped (116–120): bounded parallelism, per-path read/write locks, per-call timeout, cancellation grace, production loop wiring. Slice 255 closes AC10's background tick: `--serve` shares one strand-hosted `ToolScheduler` across automation jobs and races a `serve_scheduler_reaping` concern that periodically reaps the idle-lock table; `AgentPromptRunner` now optionally borrows an injected `{registry, scheduler}` pair. Slice 259 gives configured-channel `--serve` dispatch a per-conversation worker boundary, though the routed agent path still relies on the service-level strand instead of per-agent strands. | v1.1: dispatch singleflight + persisted index caches. Also drive the CLI agent loop on a per-agent strand and split channel agent runs onto per-agent strands where needed. | — | [spec 0012](product-specs/0012-tool-scheduler-and-state.md) |
...
2. **Runtime-service / daemon owner** — *resolved 2026-06-20*: the owner is
   ...
   **scheduler idle-lock reaping tick** — `--serve` shares one strand-hosted
   ...
   table by evicting empty conversation workers after their idle TTL, and slice
   261 adds structured worker metrics snapshots for the owner/test boundary.
   Webhook/non-chat external producers remain open.
