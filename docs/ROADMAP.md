...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 233: QQ-port milestone 3a — first `qq::QqChannel` trait-adapter surface over `GatewayTransport`, with pure `normalize_gateway_dispatch(...)` mapping C2C/group gateway message events into `channel::InboundMessage` and `Channel::next_message()` skipping unsupported non-message dispatches; gated `test-channel-qq` 51 cases / 309 assertions; `oran-channel-qq` now depends on `oran-channel`. | QQ-port milestone 3b: add the smallest outbound text send/passive-reply builder over `ApiClient` (C2C/group endpoint selection, body serialization, reply target handling, receipt parsing) before bootstrap registration. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
