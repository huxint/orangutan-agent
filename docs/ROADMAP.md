...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 237: QQ-port milestone 4b-i — enabled builds now include a hidden opt-in real QQ smoke gate for the registered path: with `ORAN_TEST_QQ_REAL_SMOKE=1` and QQ smoke env vars, `test-bootstrap` can register the configured QQ adapter, receive one real operator message, route through the configured agent bridge, observe trace/audit state, send the passive QQ reply, and shut down. Ordinary CI remains secret-free/network-free, and `channel_qq` stays default-off. | QQ-port milestone 4b-ii: run that hidden real-credential smoke gate and record the result before considering `channel_qq` default-on. | Real QQ bot credentials and a sendable operator conversation for the smoke run. | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
