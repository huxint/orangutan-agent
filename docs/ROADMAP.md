...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 235: QQ-port milestone 3c — `channels[].kind == "qq"` now has typed credential env-name / endpoint config and bootstrap registration behind `--channel_qq=y`; enabled builds own `http::Client`, `qq::TokenStore`, `qq::ApiClient`, `qq::GatewayTransport`, and `qq::QqChannel` lifetimes through an internal wrapper before registering into `ChannelManager`, while default builds skip/report QQ entries without linking adapter code. | QQ-port milestone 4: round-trip acceptance over the registered QQ path (mock gateway/API in CI plus manual real-credential smoke gate) before considering `channel_qq` default-on. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
