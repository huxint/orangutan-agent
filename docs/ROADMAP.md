...
| **Observability & trace** | Slice 243 adds the spec-0018 v1.1 SQL-derived tool-call rollup: audit DB migration version 5 creates `audit_tool_call_rollups`, and `AuditRepository::list_tool_call_rollups(...)` reads per-turn/per-tool decision counts, hook-publish counts, permitted/blocked counts, and optional `usage.wall_time_ms` latency aggregates from joined audit rows. | Refine provider streaming cancellation phases on the existing `trace_turns.cancellation_phase` field (`provider_initial`, `provider_stream`, `provider_complete`) before considering broader query language or metrics endpoints. | — | [spec 0018](product-specs/0018-first-loop-observability.md) · [`RELIABILITY.md`](RELIABILITY.md) |
