...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 231: QQ-port milestone 2b-i — the first WebSocket primitive on `oran-http` (`http::WebSocket`: `connect`/`receive`/`send_text`/`close` over libcurl connect-only mode), cancel-aware without a thread (handshake via non-blocking `curl_multi_perform` rounds; receive/send suspend on asio socket readiness over a `dup`'d socket), with the shared libcurl RAII wrappers extracted to `_impl/curl_common.hpp`; `test-http` 27 cases / 148 assertions, clean under ASan/UBSan, no new dependency. On top of slice-230's pure `qq::GatewaySession`. | QQ-port milestone 2b-ii: drive `qq::GatewaySession` over `http::WebSocket` behind `Channel::next_message()` in `oran-channel-qq` — the persistent read loop, the heartbeat timer over `async::sleep_for`, and reconnect backoff. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
