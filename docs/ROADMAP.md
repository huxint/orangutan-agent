...
| **Provider portability** | Anthropic Messages + OpenAI Responses, body + SSE streaming end-to-end in the binary (97–124), retry/fallback execution runtime, route/credential resolution, usage rollups (127), and slice 245 PascalCase tool names that satisfy Anthropic-family tool-name constraints without a provider-only alias layer. | Additional protocol families when a configured route needs one, or live-provider smoke evidence for an already configured compatible route. | A concrete demand (config naming an unsupported protocol) or secret-safe real-provider test credentials. | [`design-docs/api-portability.md`](design-docs/api-portability.md) |
