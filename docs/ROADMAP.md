...
| **Observability & trace** | Slice 242 completes spec-0018 v1.1 trace-export sinks: stdout, `--trace-export-file <path>`, and `--trace-export-post <url>` all emit the same redacted JSON Lines sequence for `--trace-export <turn-id>` and bounded list mode `--trace-export [--agent <name>] [--limit <n>]`; POST uses an explicit operator URL, suppresses stdout, and reports non-2xx responses as IO errors. | Add the SQL-derived per-turn tool-call rollup over joined audit rows before considering broader query language or metrics endpoints. | — | [spec 0018](product-specs/0018-first-loop-observability.md) · [`RELIABILITY.md`](RELIABILITY.md) |
