...
| **Workspace & path policy** | Registry pre-resolution provides pinned directory authority across approval. `FileRead`, `FileWrite`, `FileEdit`, and `FileDelete` execute through handle capabilities; FileDelete pins the approved target inode before approval and performs recursive no-follow deletion through dirfds. Shared ignore/display behavior and the read/list-only outside-workspace override are shipped. | Migrate `DirectoryList` and `FileSearch` execution, including ignore-file reads, off pathname authority. | — | [spec 0013](product-specs/0013-workspace-and-path-policy.md) |
