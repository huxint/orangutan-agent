...
| **Observability & trace** | Slice 241 extends spec-0018 v1.1 trace export with a file sink: `--trace-export-file <path>` writes the same redacted JSON Lines sequence as stdout mode for both `--trace-export <turn-id>` and bounded list mode `--trace-export [--agent <name>] [--limit <n>]`, creates parents, truncates the target file, and suppresses stdout. | Add an explicit HTTP POST sink for the same single-turn and bounded trace-export query before considering a broader query language. | — | [spec 0018](product-specs/0018-first-loop-observability.md) · [`RELIABILITY.md`](RELIABILITY.md) |
