...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 226: `oran-channel` foundation — capabilities, normalized envelopes, `Channel` trait, caller-owned `ChannelManager` (registration, one-message fan-in, send routing, lifecycle). | Smallest mock/webhook ingress adapter proving one external message reaches the manager and is handed toward the configured agent path. | — | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [active plan](exec-plans/active/2026-06-09-channel-ingress-and-adapters.md) |
