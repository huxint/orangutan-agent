...
| **Desktop app** | Slices A–C shipped (248–250): prebuilt `slint` package + `--desktop` option + gated shell + `.slint`→C++ codegen + skeleton window (A); `web`→`desktop` config migration — `DesktopConfig{enabled,theme,reduce_motion}` replacing `WebConfig` (B); always-built `oran-desktop` bridge — `ChatViewModel`, `DesktopEventSink`, `ChatBridge` (bounded UI↔runtime `async::Channel` queues + per-turn cancellation), with an injected `provider::EventSink*` on `AgentPromptRunner`; `test-desktop` 15 cases / 59 assertions vs a fake provider (C). | Slice D — chat tracer end-to-end: Slint chat UI (input, live transcript, stop) bound to the `ChatBridge`; `orangutan --desktop` builds runner + bridge and opens the working chat; spec 0007 acceptance 1–3; manual `--desktop=y` run. | None blocking. | [spec 0007](product-specs/0007-web-ui.md) · [`DESKTOP.md`](DESKTOP.md) · [active plan](exec-plans/active/2026-06-14-oran-desktop-chat-tracer.md) |
