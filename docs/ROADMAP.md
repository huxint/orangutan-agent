...
| --- | --- | --- | --- | --- |
| **Channels** *(current main line)* | Slice 238: QQ-port milestone 4b-ii prep — the hidden registered-path real-smoke test no longer requires operators to hand-supply a gateway WebSocket URL. `oran-channel-qq` now has a typed `GET /gateway/bot` discovery helper with mock HTTP coverage, and the hidden smoke uses it when `ORAN_TEST_QQ_GATEWAY_URL` is absent. Ordinary CI remains secret-free/network-free, and `channel_qq` stays default-off. | QQ-port milestone 4b-ii: run that hidden real-credential smoke gate and record the result before considering `channel_qq` default-on. | Real QQ bot credentials and a sendable operator conversation for the smoke run. | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
...
| **Tool scheduler** | Shipped (116–120): bounded parallelism, per-path read/write locks, per-call timeout, cancellation grace, production loop wiring. | Periodic `reap_idle_locks` tick — Slice C of the runtime-service-owner plan (first hoist `ToolScheduler` ownership out of `AgentPromptRunner`). | Runtime-service owner shipped (slices 253–254 `--serve`); reaping is the last remaining leg, Slice C. | [spec 0012](product-specs/0012-tool-scheduler-and-state.md) |
...
2. **Runtime-service / daemon owner** — *decision made 2026-06-18*: the owner is
   ...
   over a live-or-offline-fake provider route). The one remaining leg is the
   **scheduler idle-lock reaping tick** (Slice C — first hoist `ToolScheduler`
   ownership out of `AgentPromptRunner`); non-CLI agent-loop entry via channel
   receive/dispatch can later ride the same owner. Until C lands, scheduler reaping
   stays caller-owned by design.
