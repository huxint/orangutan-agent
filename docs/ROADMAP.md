...
| --- | --- | --- | --- | --- |
| **Channels** | Slice 260: `orangutan --serve` owns configured channel ingress with bounded per-channel+conversation dispatch queues and idle worker eviction. Buildable `config.channels[]` entries register into a strand-owned `ChannelManager`, start under the service body, pump adapter `next_message()` results into fan-in, enqueue matching automation triggers when configured, then dispatch each `(channel_id, conversation_id)` key through its own bounded worker so one slow conversation does not block unrelated conversations; an empty worker exits and is erased after its idle TTL so long-lived service state is bounded by active/recent conversations rather than every historical conversation. The QQ real-smoke gate from slice 238 remains open and `channel_qq` stays default-off. | QQ-port milestone 4b-ii once credentials exist; otherwise generic hardening: per-message deadlines, structured worker metrics, or webhook-triggered ingress. | Real QQ bot credentials and a sendable operator conversation for the QQ smoke run; no hard blocker for generic channel hardening. | [`design-docs/channel-abstraction.md`](design-docs/channel-abstraction.md) · [spec 0003](product-specs/0003-multi-platform-channels.md) · [QQ-port plan](exec-plans/active/2026-06-10-channel-qq-port.md) · [platform APIs](references/messaging-platform-apis.md) |
...
2. **Runtime-service / daemon owner** — *resolved 2026-06-20*: the owner is
   ...
   **scheduler idle-lock reaping tick** — `--serve` shares one strand-hosted
   ...
   table by evicting empty conversation workers after their idle TTL. Webhook/non-chat
   external producers remain open.
