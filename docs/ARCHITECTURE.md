...
schedule evaluation, slice 198 adds durable cron job state, and slice 199 adds
caller-driven cron scan/wait over that state, without making bootstrap open or
...
stored rows. Cron config ownership, execution/mark-fired policy, process timers,
lifecycle hook production, queueing, notifier routing, and agent firing remain
downstream.
