...
`automation.db`, exposes the migration report and repository, and constructs
retention services over that stable state. Slice 193 adds
...
`oran-bootstrap` still only maps configured retention policy into a job
descriptor; it does not automatically open `automation.db`, start timers,
acquire leases, or fire agents.
