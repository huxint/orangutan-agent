...
> concrete HTTP/TLS transport and SSE remain planned),
...
| `oran-http`          | libcurl-backed body HTTP client: `<oran/http.hpp>` exports stdlib-shaped `Header`, `BodyRequest`, `BodyResponse`, and pimpl-backed `http::Client`; callers provide the executor used for blocking curl work, curl handles remain private, and streaming/SSE plus server/router surfaces remain planned | `oran-core`, `oran-async`, libcurl |
