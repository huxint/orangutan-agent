...
> shell, the slice-73 `oran-provider` domain/cache-hint surface
> (`Request`, `Response`, `Usage`, `RetryPolicy`, `PromptCacheHints`,
> `PromptCacheOptions`, and `make_prompt_cache_hints` over
> `prompt::RenderedPrompt`, with real transports/adapters still planned),
...
| `oran-provider`      | first provider-domain slice: `<oran/provider.hpp>` exports `Request`, `Response`, `Usage`, `RetryPolicy`, `PromptCacheHints`, `PromptCacheOptions`, and `make_prompt_cache_hints(RenderedPrompt, options)`; the cache mapper validates the single prompt breakpoint, maps sections 1-6 into adapter-facing `(id, content_hash, cache_version)` keys, excludes the conversation tail, and supports route-level cache disable / minimum-prefix skip; transport, protocol adapters, execution runtime, `FakeProvider`, and real vendor mapping remain planned | currently `oran-core`, `oran-prompt`; planned `oran-async`, `oran-http` |
