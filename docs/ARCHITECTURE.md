...
| `oran-prompt`        | prompt builder skeleton: `BuilderInputs`, `BuilderOptions`, `CacheSection`, and `RenderedPrompt`; assembles the seven prompt-design sections, consumes `config::PromptActiveToolsConfig`, delegates section 2/3 catalog bytes to `tool::CatalogRenderer`, computes stable section hashes, a cache-versioned prefix hash over sections 1-6, prefix byte count, and one breakpoint before the conversation tail; planned session promotion state, real skill/memory renderers, provider cache mapping, and agent-loop prompt-cache fixture | `oran-core`, `oran-async`, `oran-config`, `oran-tool` |
