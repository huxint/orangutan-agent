...
Automation retention ownership note: slice 198 keeps automation state explicit
...
schedule evaluation, and slice 198 adds durable cron job state, without making
...
durable cron schedule and last-fired state. Cron config ownership, process
timers, lifecycle hook production, queueing, notifier routing, and agent firing
remain downstream.
