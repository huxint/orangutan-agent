...
| `oran-io`            | File/directory I/O with pathname compatibility reads/listing plus handle-authorized operations: `read_text_file_ranged(ReadOnlyFile, ...)`, `write_text_file(FileMutation, ...)`, and `delete_path(DeleteMutation, ...)`. `DirectoryAuthority` resolves beneath a pinned root; mutation capabilities reject symlinks, pin existing inodes, and revalidate namespace identity before commit. Recursive delete requires explicit intent, walks through dirfds, unlinks nested symlinks without following them, and reports removed-entry counts. The library also owns fingerprints, bounded range-read caches/singleflight, inotify invalidation, write durability, and `run_blocking`; handle-backed operations never use diagnostic paths as authorization or cache keys. Planned work includes handle-backed listing/search plus glob, pipe, subprocess, signal, and content hashing. | `oran-core`, `oran-async` |
