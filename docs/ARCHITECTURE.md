...
> watcher events), slice 52's bounded file-view cache for
...
> canonical path after explicit invalidation and watcher events), slice
...
> `WriteTextDurability` fsync policy plus dirfd-relative `FileMutation`
> commits with exclusively-created sibling temp leaves, and slice 154's public
...
> slice 266 moves recursive filesystem filtering into
> `tool::WorkspaceWalkFilter` so `FileSearch` and recursive
> `DirectoryList` share hidden-name, built-in low-signal directory,
> `.gitignore` / `.ignore`, and workspace display-label behavior;
> `entries_` uses
...
> Slice 266 adds `WorkspaceWalkFilter` plus
> `Workspace::display_path(...)`, the shared recursive filter/display
> helpers consumed by `FileSearch` and `DirectoryList`.
> Slice 267 adds the read/list-only per-call outside-workspace override:
> `FileRead`, `FileSearch`, and `DirectoryList` accept
> `allow_outside_workspace=true`; outside targets resolve only when they
> already exist, require an `ask` approval, and record the resolved absolute
> display path in audit metadata. Mutating tools keep the config-only
...
> when `DispatchContext::workspace` is supplied; slice 266 moves
> the recursive ignore stack into `WorkspaceWalkFilter` and uses
> `Workspace::display_path(...)` for workspace-backed text/data
> output paths),
...
> use `WorkspaceWalkFilter` as of slice 266, so they share
> `FileSearch`'s `.gitignore` / `.ignore` rule stack and workspace
> display labels;
...
> capability), input `{path, recursive?}` as of slice 265 —
> regular files delete directly, directories require `recursive=true`,
> registry pre-resolution pins a `DeleteMutation` before approval,
> workspace symlinks reject as `permission_denied`,
> recursive traversal never follows nested symlinks,
> `not_found` when the path does not exist, and success returns the
> literal message `deleted <path>`; slice 38 resolves through
> `tool::Workspace::resolve_delete` when supplied; slice 61 fills
> `Output::usage.bytes_written=0`, and slice 265 sets
> `files_touched` to the removed path count)),
...
| -------------------- | ----------------------------------------------- | --------------------------------------------- |
| `oran-core`          | `Result<T>`, `Error`, `Time` + ISO-8601 UTC helpers, `Role`, `StopReason`, `Content` variant, `Message`, `ToolDef` (with `required_capabilities`, `deferred`, and `category`), `core::str` UTF-8 helpers, `Capability` vocabulary (21 enumerators, slice 29 adds `list_directory`, slice 147 adds `deactivate_skill`), `core::TurnId` (slice 79 shared 16-byte trace/audit correlation id; slice 272 moves the id's identity operations into core — `format_turn_id_hex` is the canonical 32-char lowercase operator spelling shared by `--trace`, trace-export JSON, and session-store keys, and `generate_turn_id` is the UUIDv4-shaped generator previously duplicated across `oran-agent` / `oran-bootstrap`), and (slice 44) the generic `core::BoundedCache<Key, Value>` primitive (LRU on access, insert-based TTL, byte-budget eviction, explicit `erase_if` invalidation, customizable byte-size functor via template parameter, `Stats` accessor exposed for the future `oran-log`) | stdlib only |
...
| `oran-io`            | File/directory I/O with pathname compatibility plus handle-authorized operations: `read_text_file_ranged(ReadOnlyFile, ...)`, `write_text_file(FileMutation, ...)`, `list_directory(DirectoryAuthority, ...)`, and `delete_path(DeleteMutation, ...)`. `DirectoryAuthority` resolves beneath a pinned root; descriptor-backed listing classifies entries through `fstatat(..., AT_SYMLINK_NOFOLLOW)`, while mutation capabilities reject symlinks, pin existing inodes, and revalidate namespace identity before commit. Recursive delete requires explicit intent, walks through dirfds, unlinks nested symlinks without following them, and reports removed-entry counts. The library also owns fingerprints, bounded range-read caches/singleflight, inotify invalidation, write durability, and `run_blocking`; handle-backed operations never use diagnostic paths as authorization or cache keys. Planned work includes migrating recursive tool listing/search plus glob, pipe, subprocess, signal, and content hashing. | `oran-core`, `oran-async` |
