...
owners, slice 219 adds required cron `agent_prompt` persistence, and slice 220
adds `make_cron_prompt_handler(...)` for injected prompt execution through the
same explicit service/loop paths. Detached process timers, queueing, notifier routing, queue hold/drop
...
required prompt input, and slice 220 adds `make_triggered_prompt_handler(...)`
