...
> and the slice-72 first `oran-agent` surface (`agent::SessionState`
...
| `oran-agent`         | first session-state slice: `agent::SessionState` owns `prompt::PromotionState`, observes `tool.search` `Output::data_json`, promotes only deferred matches into the next prompt snapshot, ignores non-search / failed-search outputs, rejects malformed successful structured data without mutating state, and exposes promotion snapshots/stats/options for the future loop; the full ReAct loop, provider integration, memory, permissions, hooks, scheduler, and binary/CLI handoff remain planned | currently `oran-core`, `oran-prompt`, `oran-tool`; planned `oran-async`, `oran-provider`, `oran-memory`, `oran-permission`, `oran-hook` |
