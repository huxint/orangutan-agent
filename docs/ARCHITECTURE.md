...
> trace-disabled loop policy, and slice 83's provider/tool cancellation
> trace rows),
...
> explicitly disabled, writes one body-free `trace_turns` row before
> returning terminal-success responses, and writes provider/tool cancelled rows
