...
| `oran-async`         | asio `Runtime`, `Awaitable<T>`, bounded `Channel<T>` with non-blocking `try_send` / `try_receive`, cancel-aware `sleep_for`; mailbox policy lands in orchestration | `oran-core`, asio |
