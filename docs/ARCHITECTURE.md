...
> and the slice-72/slice-75 first `oran-agent` surfaces (`agent::SessionState`
...
> tools into one `provider::Request`, drives a supplied `provider::System`,
> and returns text-only terminal turns while loudly rejecting tool-use turns
> until the next loop slice)
> are implemented.
...
| `oran-agent`         | first loop surfaces: `agent::SessionState` owns `prompt::PromotionState`, observes `tool.search` `Output::data_json`, promotes only deferred matches into the next prompt snapshot, ignores non-search / failed-search outputs, rejects malformed successful structured data without mutating state, and exposes promotion snapshots/stats/options; `agent::Loop` owns the first fake-provider-backed turn driver (`LoopOptions`, `RunTurnInputs`, `RunTurnResult`) that builds a `prompt::RenderedPrompt`, maps prompt-cache hints through `provider::make_prompt_cache_hints`, mirrors active/promoted tools into name-sorted `provider::Request::tools`, sends exactly one request through `provider::System`, returns assembled text/usage/model/rendered-prompt/cache metadata for terminal text-style responses, forwards provider errors unchanged, and rejects `tool_use` responses with an explicit not-yet-implemented error; multi-iteration tool dispatch, scheduler, memory, permissions, hooks, turn audit rows, and binary/CLI handoff remain planned | `oran-core`, `oran-async`, `oran-prompt`, `oran-tool`, `oran-provider`; planned `oran-memory`, `oran-permission`, `oran-hook` |
