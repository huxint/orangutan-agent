...
Automation retention ownership note: slice 203 keeps automation state explicit
...
descriptors, without making bootstrap open or run automation state.
...
automatically open `automation.db`, upsert rows, start timers, start a
...
`automation.cron.jobs[]` parsing and bootstrap seed mapping. Cron seed
persistence into `automation.db`, process timers, queueing, notifier routing,
and agent firing remain downstream.
