...
ownership. Slice 206 adds cron run history rows for explicit
...
conflicts, slice 218 adds finite available-batch draining, and slice 223 adds
the composed `AutomationService` owner that drains buffered triggered work
before one cron cycle. Richer blocked-agent hold/requeue policy remains
downstream.
