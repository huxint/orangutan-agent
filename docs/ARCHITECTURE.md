...
> `ToolAfterPayload::data_json` for non-trusted-local sinks, and slice
> 152 substitutes optional redacted mutation `input_json` summaries for
> non-trusted sinks while preserving original input for trusted-local sinks;
> then captures
