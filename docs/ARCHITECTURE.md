...
| `oran-skill`         | skill catalog renderer + section-4 owner plus slice-136 markdown loader snapshot (`SkillDocument`, `SkillMetadata`, `Loader`, `load_directory`, `load_catalog`) over `<workspace>/.orangutan/skills/*.md`; watcher/invoke remain planned | `oran-core`, `oran-async`, `oran-io` |
