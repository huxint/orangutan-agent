...
Automation retention ownership note: slice 204 keeps automation state explicit
...
apply cron seed descriptors, and constructs retention services over that stable
state. Slice 193 adds
...
opened automation state can upsert those descriptors explicitly.
...
explicit seed application through caller-owned `AutomationRuntime`. Automatic
seed application, process timers, queueing, notifier routing, and agent firing
remain downstream.
