...
> `remove`, `find`, and `dispatch` does not allocate a temporary key; the
...
> 16 MiB default / hard ceiling on `max_bytes`), `file.edit`
...
> replacement output are capped by `max_bytes`), `file.search`
...
> `deleted <path>`; future direction is a unified delete tool
