...
> **Slice status (2026-05-24):** `oran-core` (now with `Error`/`Result`, the
...
> assembler that fans the same calls out to the observer; real transports
> and protocol adapters still planned),
...
> before returning parent-cancelled errors)
> are implemented.
