...
> `agent::Loop`, and retains transcript state across prompts while the ordinary
> binary path stays on the no-runner shell until real adapters exist; slice 16
...
> / fallback in `provider::execution::Runtime`; real transports and protocol
> adapter implementations still planned),
...
- `oran-bootstrap` now accepts `--config <path>` / `--config=<path>`. Explicit paths
  ...
  mode flags such as `--prompt` to `oran-cli`. `oran-cli` can delegate parsed prompts
  through `cli::run_async` when bootstrap supplies `AgentPromptRunner`, and tests use
  that seam with `provider::FakeProvider`; the ordinary binary still uses the
  no-runner shell until real provider adapters can be constructed from config.
