...
> `audit_events.parent_turn_id` for trace/audit joins, and slice 80's
> terminal-success loop writer for `trace_turns`),
...
> threads `RunTurnInputs::turn_id` into direct dispatch audit rows and clears
> stale context ids when tracing is disabled, and slice 80 writes one
> body-free `trace_turns` row before returning terminal-success responses)
> are implemented.
