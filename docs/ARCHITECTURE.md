...
above that service/runtime state. `oran-bootstrap` still only maps configured
retention policy into a job descriptor; it does not automatically open
`automation.db`, start timers, acquire leases, fire agents, or publish job
lifecycle hooks.
