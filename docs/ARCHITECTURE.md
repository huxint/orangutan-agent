...
| `oran-desktop`       | Native desktop app (Slint, in-process). Slice 248 ships the always-built bridge surface (`desktop::gui_compiled()` build-config accessor, `test-desktop`) plus the gated `--desktop=y` Slint shell — a custom prebuilt-binary `slint` package, a `.slint`→C++ codegen `before_build` rule confined to the target, the `shell::run` skeleton window, and the `orangutan --desktop` launch (default builds print a "rebuild with `--desktop=y`" error). The runtime bridge / view-model (bounded UI↔runtime queues, a `provider::EventSink` marshalling streamed deltas onto the Slint thread, `ChatViewModel`) lands in later chat-tracer slices. | currently `oran-core`; planned `oran-agent`, `oran-orchestration` |
