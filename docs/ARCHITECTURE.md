...
| `oran-automation`    | deterministic periodic schedule evaluation plus long-term memory retention request planning: `<oran/automation.hpp>` exports `PeriodicSchedule`, `PeriodicJobState`, `PeriodicEvaluation`, `LongtermMemoryRetentionPolicy`, `MemoryRetentionJob`, `MemoryRetentionPlan`, `evaluate_periodic_schedule(...)`, and `plan_memory_retention(...)`. The first slice is pure and caller-clocked: it computes due/not-due metadata, validates retention policy inputs, and produces `memory::longterm::DecayRequest` only when the cadence is due. Cron parsing, triggered jobs, `automation.db`, leases, backpressure, service-loop ownership, agent firing, and periodic `memory_decay` hook publication remain planned. | currently `oran-core`, `oran-memory`; planned `oran-agent`, `oran-storage`, `oran-async` |
