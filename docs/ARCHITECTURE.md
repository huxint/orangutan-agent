...
caller-driven cron scan/wait over that state, and slice 200 adds caller-driven
cron due execution that advances state only after a supplied handler succeeds,
without making bootstrap open or run automation state.
...
supplied payload handler and mark the due fire only after success. Cron config
ownership, process timers, lifecycle hook production, queueing, notifier
routing, and agent firing remain downstream.
