...
> on successful dispatch; slice 61 starts the built-in migration by
> filling usage counters for `file.write`, `file.edit`, and
> `file.delete` while leaving `data_json` empty;
> `entries_` uses
