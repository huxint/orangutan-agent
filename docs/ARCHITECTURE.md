...
> terminal-success loop writer for `trace_turns`, and slice 82's explicit
> trace-disabled loop policy),
...
> re-enters the provider until a terminal turn or iteration cap, and tags
> parent-cancelled provider/tool awaits with `cancellation_phase`; slice 79
> threads `RunTurnInputs::turn_id` into direct dispatch audit rows when
> tracing is enabled, clears/restores reusable context ids when tracing is
> explicitly disabled, and writes one body-free `trace_turns` row before
> returning terminal-success responses)
> are implemented.
...
| `oran-hook`          | hook bus + sink kinds: `Event` enum (41 lifecycle events), `Sink` abstract interface with `SinkKind::default_` / `SinkKind::trusted_local`, `Bus` with `bind` / `unbind` / `publish_advisory` (advisory contract — sinks observe, never veto; per-sink payload copies redact `ToolAfterPayload::data_json` for non-trusted-local sinks; sink exceptions are caught and surfaced as `Error::internal` in `PublishOutcome`), `InProcessSink` (std::function-backed, optional sink kind); planned `ShellSink` / `WebhookSink` / `LuaSink` (`sol2` feature-gated) and blocking-veto `publish_blocking` overload | `oran-core`, `oran-async` |
