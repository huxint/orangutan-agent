...
mapping and explicit persistence; a future channel/webhook producer must still
call `AutomationService::enqueue_triggered(...)` before the triggered half of
the service loop has external work.

Bootstrap service ownership note: `orangutan --serve` remains the explicit
long-lived owner for automation state. As of slice 257 it opens
...
over prompt-backed handlers. Ordinary startup still only stores mapped
descriptors on `RuntimeAssembly`; it does not open or mutate automation state.
