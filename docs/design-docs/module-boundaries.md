...
Cache policy belongs to a concrete resource owner. The unused
`core::BoundedCache` API is removed; storage retains its per-connection statement
cache. Shared convenience templates need runtime consumers before becoming core
APIs. [Error handling](../rules/error-handling.md) owns the result boundary.
