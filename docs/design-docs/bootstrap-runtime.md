...
`provider::resolve_route_profiles` before CLI handoff so profile references,
provider labels, and explicit profile-protocol spellings fail at startup instead
of at the future loop boundary while the same resolved bundle preserves endpoint
metadata for the future adapter factory. `api_key_env` remains a string field
only here: no provider credentials are read, no provider adapter is constructed,
and no agent runtime loop is started in this slice.
