...
constexpr std::size_t kAtomicTempCreateAttempts = 32U;
...
[[nodiscard]] std::string atomic_temp_process_id() {
#if defined(ORAN_IO_HAS_POSIX_FD)
...
[[nodiscard]] std::string atomic_temp_random_suffix() {
  auto entropy = std::random_device{};
  auto draw = [&entropy] {
    return static_cast<std::uint64_t>(entropy());
  };
  const auto high = (draw() << 32U) ^ draw();
  const auto low = (draw() << 32U) ^ draw();
...
/// (and therefore atomic under POSIX rename(2)). The PID + random suffix makes
/// temp names unique across processes; the writer opens the candidate with
/// exclusive create and retries on the unlikely collision. The leading `.`
/// keeps the temp out of LLM-facing directory listings that default to hiding
/// dotfiles.
[[nodiscard]] std::filesystem::path atomic_temp_path(const std::filesystem::path& target) {
  auto leaf = std::format(".{}.orangutan.tmp.{}.{}",
                          target.filename().string(),
                          atomic_temp_process_id(),
                          atomic_temp_random_suffix());
...
[[nodiscard]] core::Result<AtomicTempFile> create_atomic_temp_file(const std::filesystem::path& target,
                                                                   const std::string& original_path) {
  for (std::size_t attempt = 0; attempt < kAtomicTempCreateAttempts; ++attempt) {
    auto temp = atomic_temp_path(target);
    auto flags = O_WRONLY | O_CREAT | O_EXCL;
#if defined(O_CLOEXEC)
    flags |= O_CLOEXEC;
#endif
    errno = 0;
    const auto fd = ::open(temp.c_str(), flags, 0666);
    if (fd >= 0) {
      return AtomicTempFile{.path = std::move(temp), .fd = UniqueFd{fd}};
    }
    if (errno == EEXIST) {
      continue;
    }
    return std::unexpected(errno_io_error("failed to open atomic temp file", original_path));
  }

  return std::unexpected(core::Error::io("failed to allocate unique atomic temp path")
                             .with("path", original_path)
                             .with("attempts", std::to_string(kAtomicTempCreateAttempts)));
}

[[nodiscard]] core::Result<void> write_all_to_fd(int fd, const std::string& original_path, std::string_view contents) {
  auto* cursor = contents.data();
  auto remaining = contents.size();
  while (remaining != 0U) {
    const auto chunk = std::min(remaining, static_cast<std::size_t>(std::numeric_limits<ssize_t>::max()));
    errno = 0;
    const auto written = ::write(fd, cursor, chunk);
    if (written < 0) {
      if (errno == EINTR) {
        continue;
      }
      return std::unexpected(errno_io_error("failed while writing atomic temp file", original_path));
    }
    if (written == 0) {
      return std::unexpected(io_error("failed while writing atomic temp file", original_path));
    }
...
[[nodiscard]] core::Result<void> fsync_fd(int fd, const std::string& original_path, std::string message) {
  while (true) {
    errno = 0;
    if (::fsync(fd) == 0) {
      return {};
    ...
    if (errno != EINTR) {
      return std::unexpected(errno_io_error(std::move(message), original_path));
    }
  }
}

[[nodiscard]] core::Result<void> close_fd(UniqueFd& fd, const std::string& original_path, std::string message) {
  const auto raw_fd = fd.release();
  errno = 0;
  if (::close(raw_fd) != 0) {
    return std::unexpected(errno_io_error(std::move(message), original_path));
  }
...
[[nodiscard]] core::Result<void> fsync_parent_directory(const std::filesystem::path& target,
                                                        const std::string& original_path) {
...
#endif
  errno = 0;
  auto fd = UniqueFd{::open(parent.c_str(), flags)};
  if (fd.get() < 0) {
    return std::unexpected(errno_io_error("failed to open atomic write parent directory", original_path));
  }
  if (auto synced = fsync_fd(fd.get(), original_path, "failed to sync atomic write parent directory"); !synced) {
    return std::unexpected(std::move(synced).error());
  }
  return close_fd(fd, original_path, "failed to close atomic write parent directory");
}
#endif
...
[[nodiscard]] core::Result<void> atomic_write_blocking(const std::filesystem::path& target,
...
#if defined(ORAN_IO_HAS_POSIX_FD)
  auto temp = create_atomic_temp_file(target, original_path);
  if (!temp) {
    return std::unexpected(std::move(temp).error());
  }

  if (auto written = write_all_to_fd(temp->fd.get(), original_path, contents); !written) {
    std::error_code cleanup_ec;
    std::filesystem::remove(temp->path, cleanup_ec);
    return std::unexpected(std::move(written).error());
  }
  if (durability != WriteTextDurability::rename_only) {
    if (auto synced = fsync_fd(temp->fd.get(), original_path, "failed to sync atomic temp file"); !synced) {
      std::error_code cleanup_ec;
      std::filesystem::remove(temp->path, cleanup_ec);
      return std::unexpected(std::move(synced).error());
    }
  }
  if (auto closed = close_fd(temp->fd, original_path, "failed while closing atomic temp file"); !closed) {
    std::error_code cleanup_ec;
    std::filesystem::remove(temp->path, cleanup_ec);
  ...
  std::filesystem::rename(temp->path, target, rename_ec);
  if (rename_ec) {
    std::error_code cleanup_ec;
    std::filesystem::remove(temp->path, cleanup_ec);
    return std::unexpected(system_io_error("failed to commit atomic write", original_path, rename_ec));
  }
  invalidate_read_text_file_ranged_cache_blocking(original_path);

  if (durability == WriteTextDurability::fsync_file_and_parent) {
    if (auto synced = fsync_parent_directory(target, original_path); !synced) {
      return std::unexpected(std::move(synced).error());
    }
  }
  return {};
#else
  if (durability != WriteTextDurability::rename_only) {
    return std::unexpected(core::Error::io("atomic write durability unsupported on this platform")
                               .with("path", original_path)
                               .with("backend", "fsync"));
  }
  for (std::size_t attempt = 0; attempt < kAtomicTempCreateAttempts; ++attempt) {
    const auto temp = atomic_temp_path(target);
    if (auto written = stream_write(temp,
                                    original_path,
                                    contents,
                                    std::ios::binary | std::ios::out | std::ios::trunc | std::ios::noreplace);
        !written) {
      if (written.error().kind() == core::ErrorKind::conflict) {
        continue;
      }
      std::error_code cleanup_ec;
...
write_text_file_blocking(const std::string& path, const std::string& contents, WriteTextOptions options) {
  ...
  if (!options.atomic && options.durability != WriteTextDurability::rename_only) {
    return std::unexpected(core::Error::invalid_argument("write durability requires atomic write").with("path", path));
  }
  ...
  try {
    ...
    if (options.atomic) {
      return atomic_write_blocking(fs_path, path, contents, options.durability);
    }
